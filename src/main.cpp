...
int main()
{
	...
	*/
	...
	IGUIFont* font = env->getFont("textures/fonthaettenschweiler.bmp");
	//IGUIFont* font = env->getFont("textures/textp.tga");
	if (font)
		skin->setFont(font);
	...
	*/
	//env->addImage(driver->getTexture("textures/inter01a-i.tga"),
	//env->addImage(driver->getTexture("textures/inter01-i.png"),
	env->addImage(driver->getTexture("textures/inter01-i.png"),
			position2d<int>(0,0));
	...
	*/	
	...
	env->addButton(rect<s32>(310,280,410,280 + 32), 0, GUI_ID_NEW_WINDOW_BUTTON,
			L"OK");
	env->addButton(rect<s32>(410,280,510,280 + 32), 0, GUI_ID_NEW_WINDOW_BUTTON,
			L"Appearance");
	env->addButton(rect<s32>(110,280,210,280 + 32), 0, GUI_ID_NEW_WINDOW_BUTTON,
			L"New Window", L"Launches a new Window");
	env->addButton(rect<s32>(110,320,210,320 + 32), 0, GUI_ID_FILE_OPEN_BUTTON,
			L"File Open", L"Opens a file");
	env->addButton(rect<s32>(370,560,580,610 + 32), 0, GUI_ID_QUIT_BUTTON,
			L"Delete player", L"Deletes the player from the list");
	env->addButton(rect<s32>(600,560,720,610 + 32), 0, GUI_ID_QUIT_BUTTON,
			L"Cancel");
	...
	*/
	...
	env->addStaticText(L"Transparent Control:", rect<s32>(150,50,350,70), true);
	IGUIScrollBar* scrollbar = env->addScrollBar(true,
			rect<s32>(150, 75, 350, 90), 0, GUI_ID_TRANSPARENCY_SCROLL_BAR);
	scrollbar->setMax(255);

	// set scrollbar position to alpha value of an arbitrary element
	scrollbar->setPos(env->getSkin()->getColor(EGDC_WINDOW).getAlpha());

	env->addStaticText(L"Logging ListBox:", rect<s32>(150,110,350,130), true);
	IGUIListBox * listbox = env->addListBox(rect<s32>(150, 140, 350, 210));
	env->addStaticText(L"Player name", rect<s32>(210,290,350,310), true);
	env->addEditBox(L"Editable Text", rect<s32>(370, 280, 580, 320));
